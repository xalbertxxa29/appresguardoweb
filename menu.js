document.addEventListener('DOMContentLoaded', async () => {
   ...
   // --- 3. LÓGICA DE AUTENTICACIÓN Y SEGURIDAD DE PÁGINA ---
    ...
    // Renderiza la lista de últimas incidencias
    function renderIncidentsList(incidents) {
        const list = document.getElementById('incidents-list');
        if (!list) return;
        list.innerHTML = '';
        if (incidents.length === 0) {
            list.innerHTML = '<li>No hay incidencias recientes.</li>';
            return;
        }
        incidents.forEach(incident => {
            ...
            li.innerHTML = `
                <div class="item-main">
                    <span class="item-title">${incident.descripción || "Descripción no disponible"}</span>
                    <span class="item-subtitle">Por: ${incident.user || "Desconocido"}</span>
                </div>
                <div class="item-side">${new Date(incident.timestamp.seconds * 1000).toLocaleDateString('es-PE')}</div>
            `;
            list.appendChild(li);
        });
    }
    ...
    function renderRankingTable(rankingData) {
        ...
        if (rankingData.length === 0) {
            tableBody.innerHTML = '<tr><td colspan="3">No hay datos de checklists.</td></tr>';
            return;
        }
    ...
    async function loadDashboard() {
        ...
        contentArea.innerHTML = `
            <div id="dashboard-view" class="dashboard-view">
                <div class="kpi-row">
                    <div class="kpi-card"><span class="kpi-icon material-symbols-outlined" style="background-color: rgba(40, 167, 69, 0.2); color: var(--status-green);">shield_person</span><div class="kpi-info"><div id="kpi-resguardo" class="kpi-number">...</div><div class="kpi-title">Checklists Resguardo</div></div></div>
                    <div class="kpi-card"><span class="kpi-icon material-symbols-outlined" style="background-color: rgba(0, 123, 255, 0.2); color: var(--primary-blue);">directions_car</span><div class="kpi-info"><div id="kpi-conductor" class="kpi-number">...</div><div class="kpi-title">Checklists Conductor</div></div></div>
                    <div class="kpi-card"><span class="kpi-icon material-symbols-outlined" style="background-color: rgba(23, 162, 184, 0.2); color: #17a2b8;">fitness_center</span><div class="kpi-info"><div id="kpi-ejercicios" class="kpi-number">...</div><div class="kpi-title">Ejercicios Registrados</div></div></div>
                    <div class="kpi-card"><span class="kpi-icon material-symbols-outlined" style="background-color: rgba(220, 53, 69, 0.2); color: var(--status-red);">report</span><div class="kpi-info"><div id="kpi-incidencias" class="kpi-number">...</div><div class="kpi-title">Total Incidencias</div></div></div>
                </div>
                ...
                <div class="details-row">
                    <div class="widget"><div class="widget-header">Últimas Incidencias</div><ul class="details-list" id="incidents-list"></ul></div>
                    <div class="widget"><div class="widget-header">Ranking de Agentes por Checklists</div><table class="custom-table"><thead><tr><th>Rango</th><th>Agente</th><th>Total</th></tr></thead><tbody id="ranking-table-body"></tbody></table></div>
                </div>
            </div>
            <div id="data-view" class="hidden"></div>`;
        
        // --- PROCESAMIENTO DE DATOS ---
        const [resguardoSnap, conductorSnap, ejerciciosSnap, incidenciasSnap] = await Promise.all([
            db.collection('checklists').get(),
            db.collection('conductor').get(),
            db.collection('ejercicios').get(),
            db.collection('incidencias').orderBy('timestamp', 'desc').get()
        ]);

        // KPIs
        document.getElementById('kpi-resguardo').textContent = resguardoSnap.size;
        document.getElementById('kpi-conductor').textContent = conductorSnap.size;
        document.getElementById('kpi-ejercicios').textContent = ejerciciosSnap.size;
        document.getElementById('kpi-incidencias').textContent = incidenciasSnap.size;
        ...
        incidenciasSnap.forEach(doc => {
            const incidentDate = doc.data().timestamp.toDate();
        ...
        const recentIncidents = incidenciasSnap.docs.map(doc => doc.data()).slice(0, 5);
